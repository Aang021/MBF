...
#Be yourself and never surentod, Kalian bisa jadi pro player asal bisa jump shot di epep
#Udah jangan diubah lagi ntar eror, tinggal pake aja kok ribet amat anjenkk
P = "\x1b[0;97m" # Putih
M = "\x1b[0;91m" # Merah
H = "\x1b[0;92m" # Hijau
K = "\x1b[0;93m" # Kuning
...
\x1b[0;96m /_/ /_/ /_/_____/_/\x1b[0;97mv10.4\x1b[0;96m|| UPDATE 2022
\x1b[0;96m╔════════════════════════════════════════════════╗
...
def menu_log():
    ...
    banner()
    var_menu()
    pmu = input('%s╠══[%s•%s] %sPilih : '%(O,K,O,K))
    print('%s║'%(O))
    if pmu in ['']:
...
def menu():
    ...
    try:
        ...
        status = '\x1b[0;96mLu Jomblo Kentod'
        users = '-'
        maile = '-'
        kunci = '-'
        bergabung = '-'
        ...
        while True:
            ...
            if f=="":
                ...
                try:
                    ...
                    self.fl = []
                    start_methodezz()
                    kopi = input('%s╠══[%s•%s] %sPilih : '%(O,K,O,K))
                    if kopi in ['']:
                        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
                        menu()
                    elif kopi in ['1','01']:
                        ...
                        menu()
                    start_method()
                    put = input('%s╠══[%s•%s] %sPilih : '%(O,K,O,K))
                    print('%s║'%(O))
                    if put in ['']:
                        jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
                        menu()
                        ...
                        print('%s╠══[%s?%s] %sMunculkan Opsi Checkpoin? [y/n]'%(O,K,O,H))
                        puf = input('%s╠══[%s•%s] %sPilih : '%(O,K,O,K))
                        if puf in ['']:
                            jalan('%s╚══[%s!%s] %sIsi Yang Benar'%(M,P,M,P))
                            menu()
                        elif puf in ['1','01','001','y','Y']:
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%CP%s] %s • %s • %s %s %s%s"%(O,K,O,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        m,d,y = ttl.split("/")
                        m = bulan_ttl[m]
                        print("\r%s[%sCP%s] %s • %s • %s %s %s%s"%(O,KO,fl.get("id"),i,d,m,y,tahun(fl.get("id"))))
                        self.cp.append("%s•%s•%s%s%s"%(fl.get("id"),i,d,m,y))
                        open("CP/%s.txt"%(tanggal),"a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y))
                        break
                    except(KeyError, IOError):
...
def var_ugen():
    ...
    print("%s╠══[%s2%s] %sGanti User Agent %s[%s Manual %s]"%(O,K,O,H,O,P,O))
    print("%s╠══[%s3%s] %sGanti User Agent %s[%s Sesuaikan HP %s]"%(O,K,O,H,O,P,O))
    print("%s╠══[%s4%s] %sHapus User Agent"%(O,K,O,M))
    print("%s╠══[%s5%s] %sCek User Agent"%(O,K,O,H))
    print("%s╠══[%s0%s] %sKembali"%(O,K,O,K))
def start_method():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sMetode B-Api \x1b[0;96m[ \x1b[0;93mServer Sedang Eror \x1b[0;96m]'%(O,K,O,H))
...
if __name__=='__main__':
  ...
  menu()
#Aangxd_X_Lauraa<3
#Lo_Kentod_Anjing_Kontol_Ngentod!!!!
#Be_Yourself_And_Never_Surentod_Terkentod_Kentod.