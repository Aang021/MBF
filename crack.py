...
class crack:
    ...
    def api(self,fl):
        try:
            ...
            self.ko+=1
            print("\r%s[%s%s%s][%s%s/%s%s][%sOK:%s%s][%sCP:%s%s]%s"%(K,P,fl.get("id"),K,P,self.ko,len(self.fl),O,P,len(self.ada),O,P,len(self.cp),O,P), end='');sys.stdout.flush()
        except:
            self.api(fl)
...
def start_method():
    print('%s║'%(O))
    print('%s╠══[%s1%s] %sMetode Api v2'%(O,P,O,P))
